...
export const ru: TranslationStructure = {
    ...
    newSession: {
        ...
        preset: {
            ...
            noProfiles: 'На этой машине нет пресетов. Определите их в ~/.happy/profiles.json на хосте (см. документацию демона).',
    ...
    fleet: {
        ...
        cleanup: {
            ...
            confirmBody: ({ count }: { count: number }) => `Архивировать неактивные сессии старше 7 дней (${count})?`,
            done: ({ count }: { count: number }) => `Заархивировано сессий: ${count}`,
            doneTitle: 'Очистка завершена',
            nothingTitle: 'Нечего очищать',
            nothingBody: 'Нет неактивных сессий старше 7 дней.',
        },
    },
    ...
    run: {
        ...
        dispositionTopicHint: 'Необязательно. Гейт уверенности использует это, чтобы определить автономность (продолжить / под контролем / удержать). Если оставить пустым или указать неизвестную гейту тему, задача будет удержана для одобрения перед каждым запуском.',
        ...
        gateActionProceed: 'Выполнять автономно',
        gateActionProceedSupervised: 'Выполнять (под надзором)',
        gateActionEscalate: 'Спросить перед запуском',
        gateActionHold: 'Ожидает подтверждения',
        gateBucketHighTrust: 'Высокое доверие',
        gateBucketModifyProne: 'Часто корректируется',
        gateBucketMixed: 'Смешанно',
        gateBucketOverrideProne: 'Часто отклоняется',
        gateBucketThin: 'Недостаточно данных',
        resolveGateFailed: 'Не удалось обработать',
        resolveGateFailedMessage: 'Эта задача больше не ожидает вашего решения — возможно, она уже обработана или изменена.',
    },
    ...
    cron: {
        ...
        deleteError: 'Не удалось удалить расписание. Попробуйте ещё раз.',
        ...
        cronExprInvalid: 'Выражению cron нужно пять полей, разделённых пробелами (напр. */30 * * * *).',
        promptPlaceholder: 'Опишите, что вы хотите, чтобы агент сделал…',
    ...
    event: {
        ...
        statusEnabled: 'Включено',
        statusDisabled: 'Отключено',
        submitSuccess: 'Подписка создана.',
