...
export const zhHant: TranslationStructure = {
    ...
    newSession: {
        ...
        createDirectoryBody: ({ directory }: { directory: string }) => `目錄「${directory}」不存在。要建立它嗎？`,
        preset: {
            tabCustom: '自訂',
            tabPreset: '預設',
            chooseProfile: '選擇預設',
            noProfiles: '此裝置上沒有預設。請在主機的 ~/.happy/profiles.json 中定義（參閱常駐程式文件）。',
    ...
    fleet: {
        ...
        cleanup: {
            ...
            doneTitle: '清理完成',
            nothingTitle: '沒有需要清理的工作階段',
            nothingBody: '沒有超過 7 天的非活躍工作階段。',
        },
    },
    ...
    subscriptions: {
        title: '訂閱',
        machineRowTitle: '訂閱',
        machineRowSubtitle: '此機器上的 Claude 帳戶',
        sectionAccounts: '帳戶',
        accountsFooter: '此機器上有多個 Claude 訂閱。雲端工作階段會透過所選帳戶路由；未選擇時使用預設帳戶。',
        defaultBadge: '預設',
        noAccounts: '尚無帳戶',
        noAccountsSubtitle: '新增一個 Claude 訂閱，以透過它路由雲端工作階段。',
        addAccount: '新增帳戶',
        usage5h: '5 小時',
        usage7d: '7 天',
        usageUnknown: '用量未知',
        accountActionsTitle: ({ name }: { name: string }) => `帳戶：${name}`,
        setDefault: '設為預設',
        remove: '移除帳戶',
        removeConfirmTitle: '移除帳戶？',
        removeConfirmMessage: ({ name }: { name: string }) => `「${name}」將從此機器的保險庫中移除。正在執行的工作階段會繼續運作，直到被切換。`,
        migrateRow: '遷移正在執行的工作階段',
        addTitle: '新增帳戶',
        addInstructionsTitle: '取得設定權杖',
        addInstructionsRun: '在機器上執行 `claude setup-token` 並登入。',
        addInstructions: '將權杖貼到下方。它會被加密到保險庫中，絕不離開守護行程。',
        nameLabel: '名稱',
        namePlaceholder: '例如：工作、個人',
        nameRequired: '請為此帳戶輸入名稱。',
        tokenLabel: '設定權杖',
        tokenPlaceholder: '在此貼上 claude setup-token',
        tokenRequired: '請貼上設定權杖。',
        makeDefault: '設為預設',
        addButton: '新增帳戶',
        migrateTitle: '遷移工作階段',
        migrateInstructions: '選擇正在執行的工作階段並將其切換到另一個帳戶。切換是即時的——無需重新啟動。',
        migrateNoSessions: '此機器上沒有正在執行的工作階段。',
        migrateUnbound: '未綁定到帳戶',
        migrateSelectTarget: '切換到',
        migrateButton: ({ count }: { count: number }) => `切換 ${count} 個工作階段`,
        migrateResult: ({ remapped, skipped }: { remapped: number; skipped: number }) => `已切換 ${remapped} 個，略過 ${skipped} 個。`,
    ...
    run: {
        ...
        fieldTimeout: '逾時 (分鐘)',
        tierTrusted: '受信任 (自主)',
        tierSupervised: '監督模式 (需確認)',
        stop: '停止',
        stopConfirm: '停止此執行中的任務？程序將收到 SIGTERM，5秒後收到 SIGKILL。',
        statusPending: '等待中',
        statusRunning: '執行中',
        statusSucceeded: '已成功',
        statusFailed: '已失敗',
        ...
        attempts: ({ n, max }: { n: number; max: number }) => `第 ${n}/${max} 次嘗試`,
        costLine: ({ usd }: { usd: string }) => `費用: $${usd}`,
        budgetLine: ({ usd }: { usd: string }) => `預算: $${usd}`,
        exitReason: ({ reason }: { reason: string }) => `結束: ${reason}`,
        openSession: '開啟工作階段',
        escalation: '此任務需要您的批准',
    ...
    cron: {
        ...
        fieldTurns: '最大輪數',
        fieldTimeout: '逾時（分鐘）',
        fieldAllowedTools: '允許的工具',
        tierTrusted: '受信任（自主）',
        tierSupervised: '受監督（詢問）',
        delete: '刪除排程',
        deleteConfirm: '刪除此排程？將不再排入後續執行。',
        deleteError: '無法刪除此排程。請重試。',
        statusEnabled: '已啟用',
        statusDisabled: '已停用',
        cronExprPlaceholder: '例如 */30 * * * *',
        cronExprHint: '五欄位 cron 運算式，依守護行程時區計算。',
        cronExprInvalid: 'cron 運算式需要五個以空格分隔的欄位（例如 */30 * * * *）。',
        promptPlaceholder: '描述您希望代理執行的操作…',
    ...
    event: {
        ...
        tierSupervised: '受監督（詢問）',
        delete: '刪除訂閱',
        deleteConfirm: '刪除此訂閱？將不再觸發任何執行。',
        deleteError: '無法刪除此訂閱。請重試。',
        statusEnabled: '已啟用',
        statusDisabled: '已停用',
        submitSuccess: '已建立訂閱。',
        eventTypeHint: '此版本僅支援 git.commit。',
        matchKeyPlaceholder: '選用，例如分支名稱',
        matchKeyHint: '僅當事件負載與此鍵相符時才觸發。留空以比對全部。',
        promptPlaceholder: '描述您希望代理執行的操作…',
        allowedToolsPlaceholder: '選用，以逗號分隔',
        allowedToolsHint: '將代理限制為這些工具。留空以使用預設的預設值。',
        fieldDispositionTopic: '處置主題',
