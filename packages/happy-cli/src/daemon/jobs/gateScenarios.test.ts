...
describe('E05 gate scenarios (TS-01..13, executable mirror of tests/test-scenarios.md)', () => {
  ...
  function scheduler(spawn: (o: SpawnSessionOptions) => Promise<SpawnSessionResult>, loadRollup: () => DispositionRollup | null = scenarioRollup) {
    // The E05 fixtures denote a worktree with `mkdir('.git')` (gitDir()), the old
    // AC-3 semantics. Inject a gitContainment probe that mirrors that existsSync
    // check so these scenarios stay decoupled from production's real `git
    // rev-parse` worktree/branch probe (covered directly in scheduler.test.ts).
    // Branch is always a non-protected feature branch here.
    const gitContainment = (d: string) => ({ isWorktree: existsSync(join(d, '.git')), branch: 'feature/x' })
    return new JobScheduler({ store, localSemaphore: new Semaphore(1), spawn, loadRollup, gitContainment })
  }
  ...
  it('TS-11 — SEC-002: approve cannot bypass AC-3 containment (trusted without a worktree stays parked) (D-E05-10)', async () => {
    ...
    expect(after.gateReason).toBe('approve refused: trusted-requires-worktree (AC-3 / D-E04-2)')
  })
